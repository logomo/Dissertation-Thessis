...
\begin{equation}\label{eq:trajectoryTrackingOptimalizaitonProblem}
    \begin{aligned}
         &\texttt{Minimize:} && costOfTrajectoryFlown &(eq. \ref{eq:consFunctionMeta})\\
         &\texttt{Minimize:} && reference Trajectory Deviation
         &(eq. \ref{eq:ReferenceTrajectoryDeviation})\\
         &\texttt{Subject to:} &&&\\
         &&&UAS\text{ }Dynamics&(eq. \ref{eq:madSystemdefinition})\\
         &&&MovementAutomatonControl&\begin{gathered}(eq. \ref{eq:madInitialState})\\\vdots\\(eq. \ref{eq:madStateProjection})\end{gathered}\\
         &&&Mission&(eq. \ref{eq:missionAbstractSet})\\
         &&&KnownWorld(t)&(eq. \ref{eq:knownWorldSet})\\
         &&&SafetyMargin(t)&(eq. \ref{eq:safetyMarginAbstract})\\
         &&&HardFlightConstraints&(eq. \ref{eq:HFlightConstraints})\\
         &&&SoftFlightConstraints&(eq. \ref{eq:SFlightConstraints})\\
         &&&HardSpaceConstraints&(eq. \ref{eq:hardSpaceConstraints})\\
         &&&SoftSpaceConstraints&(eq. \ref{eq:softSpaceConstraints})\\
    \end{aligned}
\end{equation}
