...
\begin{enumerate}
	...
	\begin{enumerate}[a]
		\item \emph{Avoidance mode} is active when the \emph{UAS} is nearby to obstacle ($distance$ $(obstacleCenter,$ $UASPosition)$ $\le$ $20m$).
...
\begin{enumerate}
	...
	\item \emph{$2^{nd}$ building avoidance.} (fig. \ref{fig:secondBuildingAvoidance}) - UAS avoids the building from the right side while avoiding active non-convex portion of the building.  
...
\noindent\paragraph{Path Tracking Deviations:} Deviations (tab. \ref{tab:pathTrackingParametersForBuildingAvoidance}) from \emph{reference trajectory} are in expected ranges considering the  \emph{mission plan} (tab. \ref{tab:missionSetupForBuildingAvoidanceScenario}) and \emph{obstacle properties} (tab. \ref{tab:obstacleSetBuildingAvoidance}).
\begin{table}[H]
	\centering
