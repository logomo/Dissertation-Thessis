\subsection{\secState{D}Storm}\label{s:testStorm}
    ...
    The \emph{Storm} is moving through an \emph{operaitonal region} with linear velocity $0.5 ms^{-1}$. The \emph{storms center} was first detected at \emph{decision frame} $0$ at position $[0,50,0]^T$.
    ...
    \paragraph{Assumption:} Every \emph{avoidable moving constraint} is usually slower than an \emph{Approaching UAS}, or its radius is smaller than turning radius of an \emph{Approaching UAS}.
    ...
    \paragraph{Simulation run:} \emph{Notable moments} from a \emph{simulation run} (fig. \ref{fig:testCaseStormAvoidance}) are following:
    \begin{enumerate}
