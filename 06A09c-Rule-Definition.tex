\subsection{\secState{R}Rule: Detect Collision Cases}\label{sec:detectCollisionCases}
...
\subsection{\secState{R}Rule: Resolve Collision Case}\label{sec:ruleResolveCollisionCase}
...
\subsection{\secState{R}Rule: Close Collision Cases}\label{sec:ruleCloseCollisionCases}    
...
\subsection{\secState{R}Rule: Head on Approach}\label{sec:ruleHeadOnApproach}
...
\noindent \emph{Virtual round-abound center} is calculated as \emph{corrected median} (\ref{eq:roundaboundCorrectedMedian}) taking \emph{cluster of collision cases} and calculates median of their collision points corrected by \emph{weather} and \emph{wake turbulence} factor.
...
\subsection{\secState{R}Rule: Converging Maneuver}\label{sec:ruleConvergingManuever}
...
\subsection{\secState{R}Rule: Overtake}\label{sec:ruleOvertake}
...
\subsection{\secState{R}Rule: Right Plane Heading}\label{sec:ruleRightPlaneHeading}
...
\newpage
\subsection{\secState{R}Rule: Enforce safety margin}\label{sec:ruleEnforceSafetyMargin}
...
Rule for safety margin check is summarized in (tab. \ref{tab:ruleEnforceSafetyMargin}).   
